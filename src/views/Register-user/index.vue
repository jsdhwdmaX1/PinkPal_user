...
const rules = {
  ...
  password: [
    ...
    { max: 16, message: '密码长度不超过16个字符', trigger: 'blur' },
  ],
  agree: [
    {
...
const doLogin = () => {
  ...
  formRef.value.validate(async (valid) => {
    ...
    if (valid) {
      ...
      // 2. 跳转首页
      router.replace({ path: '/login-user' })
    }
  })
}
          ...
          <div class="form">
            <el-form ref="formRef" :model="form" :rules="rules" label-position="right" label-width="80px" status-icon>
              <el-form-item prop="username" label=" 账 户 ">
                <el-input v-model="form.username" clearable placeholder="请输入账户" />
              </el-form-item>
            ...
            <div style="text-align: right; margin-top: 30px;">
              <RouterLink to="/login-user">没有账号?去登录</RouterLink>
            </div>
          </div>
        </div>
      </div>
